...
// CUSTOM: use local AgenticWallet-compatible wallet-v5 wrappers/helpers.
import { Opcodes } from './custom-agentic-wallet-v5';
...
describe('Wallet v5 external tests', () => {
    ...
    let walletId: bigint; // CUSTOM: AgenticWallet binds signed requests to uint256 nftItemIndex.
    const validOpCodes = [
        Opcodes.auth_signed,
    ...
    let assertSendMessages: (
        exp: number,
        wallet_id: bigint, // CUSTOM: AgenticWallet wallet id is uint256.
        valid_until: number,
        seqno: bigint | number,
        messages: MessageOut[],
        key: Buffer,
    ...
    type TestArgs = {
        walletId: bigint; // CUSTOM: AgenticWallet wallet id is uint256.
        valid_until: number;
        seqno: bigint | number;
    ...
    */
    ...
    beforeAll(async () => {
        ...
        code = await compile('AgenticWallet'); // CUSTOM: compile AgenticWallet instead of wallet_v5.
        ...
        wallet = blockchain.openContract(
            TestWalletFromV5(
                WalletV5Test.createFromConfig({
                    // CUSTOM: use initialized AgenticWallet storage as wallet-v5 compatibility state.
                    ...
                    extensions: Dictionary.empty(Dictionary.Keys.BigUint(256), Dictionary.Values.BigInt(1)),
                }, code, 0)
            )
        );
        ...
        testSendInit = async (shouldWork, validateNewWallet) => {
            ...
            */
            const newTestWallet = WalletV5Test.createFromConfig({
                // CUSTOM: create initialized AgenticWallet-compatible wallet instead of WalletV5.create().
                signatureAllowed: true,
                seqno: 0,
                publicKey: keys.publicKey,
                extensions: Dictionary.empty(Dictionary.Keys.BigUint(256), Dictionary.Values.BigInt(1)),
            ...
            const newAddress = contractAddress(-1, { code, data: newTestWallet.init!.data }); // CUSTOM: compat wrapper always has init when created from config.
                    ...
                    wallet: [
                        {
                            ...
                            outMsg: internal_relaxed({
                                ...
                                init: {
                                    code,
                                    data: newTestWallet.init!.data // CUSTOM: compat wrapper always has init when created from config.
                                }
                            }),
                            mode: defaultExternalMode
                        }
    ...
    describe('Actions', () => {
                ...
                await testSetCode(
                    async args => {
                        ...
                        expect(res.transactions).toHaveTransaction({
                            ...
                            exitCode: 147 // CUSTOM: AgenticWallet maps malformed c5 to ERROR_INVALID_C5 (147), not VM underflow 9.
                        });
        ...
        describe('Internal', () => {
                ...
                await testSetCode(
                    async args => {
                        ...
                        expect(res.transactions).toHaveTransaction({
                            on: wallet.address,
                            op: Opcodes.auth_signed_internal,
                            outMessagesCount: 0, // CUSTOM: AgenticWallet keeps internal signed action-phase failures non-bounced.
                            aborted: false, // CUSTOM
                            success: true, // CUSTOM
                            exitCode: 147 // CUSTOM: AgenticWallet maps malformed c5 to ERROR_INVALID_C5 (147), not VM underflow 9.
                        });
                        expect(await wallet.getSeqno()).toEqual(Number(args.seqno) + 1); // CUSTOM: seqno is committed before action processing.
                    },
                    async args => {
                        if (args.actions == undefined || args.key == undefined) {
                            throw new Error('Actions and keys are required');
            ...
            it('should ignore message if length less than required for signature check', async () => {
                ...
                // CUSTOM: AgenticWallet accepts this malformed-but-signable case without bounce.
                expect(res.transactions).toHaveTransaction({
                    on: wallet.address,
                    op: Opcodes.auth_signed_internal,
                    aborted: false,
            ...
            it('should not be able to install already installed extendsion', async () => {
                await testAddExtAlreadyIn(async args => {
                    ...
                    expect(res.transactions).toHaveTransaction({
                        ...
                        from: owner.address,
                        op: Opcodes.auth_signed_internal,
                        aborted: false, // CUSTOM: AgenticWallet keeps internal signed action-phase failures non-bounced.
                        outMessagesCount: 0, // CUSTOM
                        exitCode: ErrorsV5.add_extension
                    });
                    expect(await wallet.getSeqno()).toEqual(Number(args.seqno) + 1); // CUSTOM: seqno is committed before action processing.
                });
        ...
        describe('Malformed action list', () => {
            ...
            it('should reject action list with extra data/refs', async () => {
                ...
                for (let payload of [excessiveData, truncated, extraRef, noRef]) {
                    ...
                    seqNo += 1; // CUSTOM: internal signed path commits seqno before action processing.
            ...
            */
            ...
            for (let action of [disableSigAuth, enableSigAuth]) {
                ...
                for (let actionSet of [actionSingle, actionFirst, actionLast, actionRandom]) {
                    ...
                    const intRes = await wallet.sendInternalSignedMessage(owner.getSender(), {
                        value: toNano('1'),
                        body: msgInt
                    });
                    expect(intRes.transactions).toHaveTransaction({
                        on: wallet.address,
                        aborted: false, // CUSTOM: AgenticWallet keeps internal signed action-phase failures non-bounced.
                        exitCode: ErrorsV5.only_extension_can_change_signature_mode
                    });
                    expect(await wallet.getSeqno()).toEqual(++seqNo); // CUSTOM: seqno is committed before action processing.

                    const msgExt = WalletV5Test.requestMessage(
                        false,
                        walletId,
                        curTime() + 100,
                        seqNo, // CUSTOM: rebuild external message with updated seqno after internal commit.
                        { extended: actionSet },
                        keys.secretKey
                    );
                    const extRes = await wallet.send(msgExt);
